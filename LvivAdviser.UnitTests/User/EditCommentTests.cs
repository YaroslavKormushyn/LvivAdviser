	...
	public class EditCommentTests
	{
		...
		public void EditComment()
		{
			...
			mockRatingRepository
				...
				.Returns(ratingsList
					.First(rating => rating.ContentId == content.Id));
			...
			var viewResult = userController.EditComment(content.Id);

			Assert.AreEqual(
				ratingsList.First().Comment,
				((CommentEditModel)viewResult.Model).Comment);
			Assert.AreEqual(
				ratingsList.First().UserId,
				((CommentEditModel)viewResult.Model).UserName);
			Assert.AreEqual(
				ratingsList.First().ContentId,
		...
		[TestMethod]
		public void EditInvalidIdentityComment()
		{
			var content = new Content
			{
				Id = 1
			...
			Assert.AreEqual(
				"Cannot edit comments from others.",
		...
		public void UpdateComment()
		{
			...
			userController.Validate(model);
			var actionResult = userController.EditComment(model);
